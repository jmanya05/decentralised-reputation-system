""" Proofs for reputation voting signature """
...
def make_proof_credentials_reputation(params, aggr_vk, sigma, private_m, UUID):
    ...
    rt = wt - c*t % o
    pi_reputation = (c, rm, rt)

    ## output
    return (kappa, nu, sigma_prime, zeta, pi_reputation)

def verify_proof_credentials_reputation(params, aggr_vk, sigma, kappa, nu, zeta, pi_reputation, UUID, public_m=[]):
	""" verify reputation voting signature """
	(G, o, g1, hs, g2, e) = params
	(g2, alpha, beta) = aggr_vk
	(h, s) = sigma
	(c, rm, rt) = pi_reputation
	private_m_len = len(pi_reputation[1])
...
def make_proof_tally_reputation(params, li, enc_results, priv):
...
def verify_proof_tally_reputation(params, li, enc_results, pi_dec, eta):
